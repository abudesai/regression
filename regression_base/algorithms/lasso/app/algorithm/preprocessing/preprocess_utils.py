...
def get_cat_and_num_vars_lists(data_schema, model_cfg):  
    
    cat_vars, num_vars = [], []
    schema_keys = model_cfg["data_schema_keys"]
    for attribute in data_schema[schema_keys["attributes"]]: 
        if attribute[schema_keys['attr_type']] == schema_keys['categorical']:
            cat_vars.append(attribute[schema_keys['attr_name']])
        elif attribute[schema_keys['attr_type']] == schema_keys['numeric']:
            num_vars.append(attribute[schema_keys['attr_name']])

    # print("# cat_vars: ", len(cat_vars), "# num_vars: ", len(num_vars))
...
def get_vars_with_nas(data, pp_params):     
    ...
    # print("# cat_na: ", len(cat_na), "# num_na: ", len(num_na))
...
def get_preprocess_params(data, data_schema, model_cfg):        
    ...
    pp_params["row_id"] = data_schema[model_cfg["data_schema_keys"]["row_id"]]    
    
    # set the target attribute
    pp_params["target_attr_name"] = data_schema[model_cfg["data_schema_keys"]["target_attr_name"]]    
    
    # get the list of categorical and numeric variables and set in the params dict
    cat_vars, num_vars = get_cat_and_num_vars_lists(data_schema, model_cfg)    
    ...
    pp_params["retained_vars"] = [pp_params["row_id"]] + \
    ...
    # print("with_string_missing", pp_params["with_string_missing"])
    # print("with_freq_cat", pp_params["with_freq_cat"])
    
    # pprint.pprint(pp_params)    
    return pp_params


def save_preprocessor(preprocess_pipe, file_path_and_name):
    joblib.dump(preprocess_pipe, file_path_and_name)        
    return    
    

def load_preprocessor(file_path_and_name):
    if not os.path.exists(file_path_and_name):
        err_msg = f'''Error: No trained preprocessor found. 
